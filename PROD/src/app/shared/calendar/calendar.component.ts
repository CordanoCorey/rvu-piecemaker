...
import { LookupValue, DumbComponent, DateHelper, build, inArray, toArray, TypeConstructor, COLORS } from '@caiu/library';
...
export class CalendarComponent extends DumbComponent implements OnInit {
  ...
  @Input() endDate: Date;
  @Output() goToDay = new EventEmitter<CalendarDay>();
  ...
  onGoToDay(e: CalendarDay) {
    this.activeDate = new Date(e.date);
    this.activeDay = e;
    this.goToDay.emit(e);
